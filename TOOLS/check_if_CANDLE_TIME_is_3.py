...
"""
Читает settings.yaml, берёт значение CANDLE_TIME (формат HH:MM или H:MM),
смотрит цифру по указанному индексу в часах (0 — десятки часа, 1 — единицы часа).
Если эта цифра равна TARGET_DIGIT — в ключ FLAG_KEY записывает TRUE_WORD,
иначе FALSE_WORD. Меняет только значение, сохраняя отступы, расположение и
комментарии в строке. Если ключа нет — добавляет в конец файла.
"""
...
SETTINGS_PATH       = "./settings.yaml"   # путь к файлу
TIME_KEY            = "CANDLE_TIME"       # имя ключа с временем
FLAG_KEY            = "CANDLE_TIME_IS_3"  # имя флага для записи результата
